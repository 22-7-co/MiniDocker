...
var (
	...
	defaultNetworkPath = "/tmp/mydocker-test/network/network/"
...
func (nw *NetWork) remove(dumpPath string) error {
	if _, err := os.Stat(path.Join(dumpPath, nw.Name)); err != nil {
		if os.IsExist(err) {
			...
			return fmt.Errorf("remove path err:%w", err)
		}
	} else {
...
func enterContainerNetns(enLink *netlink.Link, cinfo *container.ContainerInfo, ep *Endpoint) func() {
	...
	if err != nil {
		log.Errorf("error get current net namespace, %v", err)
	}
	...
	if err = setInterfaceUp(ep.Device.PeerName); err != nil {
		log.Errorf("setInterfaceUp ip %s, err: %v", ep.Device.PeerName, err)
	}
	// 调用函数设置容器内的Veth端点的IP
	if err = setInterfaceIp(ep.Device.PeerName, interfaceIp.String()); err != nil {
		log.Errorf("setinterface ip %v, err: %v", ep.Network, err)
	}
	// net namespace中默认的本地地址是127.0.0.1 的lo网卡关闭状态
	// 启动它以保证容器访问自己的请求
	if err = setInterfaceUp("lo"); err != nil {
		log.Errorf("setInterfaceUp ip %s, err: %v", "lo", err)
	}
	...
	if err = netlink.RouteAdd(defaultRoute); err != nil {
		log.Errorf("add default route err: %v", err)
	}

	// 返回之前的net namespace
	// 在容器的网络空间中，执行完容器配置之后，调用此函数就可以将程序恢复到原生的net namespace
	return func() {
		// 恢复到上面获取到的net namespace
...
func configPortMapping(ep *Endpoint, cinfo *container.ContainerInfo) error {
	...
	log.Infof("container name: %s", cinfo.Name)
	// 遍历容器端口映射列表
	for _, pm := range ep.PortMapping {
		// 分配宿主机的端口和容器的端口
		portMapping := strings.Split(pm, ":")
		if len(portMapping) != 2 {
			log.Errorf("port mapping err: %s", pm)
			continue
		}
		...
		iptableCmd := fmt.Sprintf("-t nat -A PREROUTING -p tcp -m tcp --dport %s -j DNAT --to-destination %s:%s", portMapping[0], ep.IpAddress.String(), portMapping[1])
		log.Infof("%v", iptableCmd)
		// 执行iptables命令，添加端口映射和转发规则
		cmd := exec.Command("iptables", strings.Split(iptableCmd, " ")...)
		output, err := cmd.Output()
		if err != nil {
			log.Errorf("iptables err %v, %v", output, err)
			continue
		}
	}
	return nil
...
func Connect(networkName string, cinfo *container.ContainerInfo) error {
	...
	ep := &Endpoint{
		...
		IpAddress:   ip,
		Network:     &network,
		PortMapping: cinfo.PortMappings,
	}
	...
	if err := configEndpointIpAddressAndRoute(ep, cinfo); err != nil {
		log.Errorf("config endpoint ip address: %v", err)
		return err
	}
	return nil
}
